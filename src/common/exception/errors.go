...
var InventoryError = errors.New("Not enough inventory for this shipment")
var separator = ":\n\t"

const (
	traceStart = 4 // ignore the generating functions in generator.go
	traceEnd   = 64
)
...
func (e *Exception) Error() string {
	...
	// fmt.Println(e.stackPrint)
	return e.err.Error()
}

// Error logs to the client.
func LogErr(err error) string {
	...
	return fmt.Errorf("query %s failed with args %v, err: %w", exception.query, exception.args, exception.err).Error()
}
...
func (e *Exception) writeTraceToSb(sb *strings.Builder) {
	if e.stack == nil {
		return
	}
